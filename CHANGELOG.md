...
- 增强面板新增「再细化…」入口：默认轻路径只保留草稿状态与结果预览，点「再细化…」展开双栏全配置（档位、上下文、强度、方法切换）；已保存「完整模式」旧偏好的用户打开面板即进入展开态。
...
- 移除极简/完整「界面模式」设置与首次体验自动展开逻辑：只保留一套默认轻路径 + 「再细化…」展开的稳定界面，`display-mode.v1` 中旧的 `full` 偏好仍被读取作为展开态初始值；首次体验进度仍记录 0~3 次成功但不再驱动界面切换。
