...
- 移除「界面模式」设置（自动/极简/完整三档）及其全部兼容读取：不再读取 `display-mode.v1`，也不保留「再细化…」入口。插件始终使用单一完整配置界面。

### 新增

- 流式增强阶段监控：面板阶段为「等待模型响应 → 输出中 → 完成」；只有用户开启可选五维诊断时才增加「诊断中」。流式期间实时计时。自定义流式 adapter 可选实现 `onStage(phase, model)`；`streamEnhanceWithCurrentSessionModel` 提供 `onEvent` 过程回调（`start` / `first-token` / `prompt-start` / `finish`），不传时零开销。
...
- 极简/完整三档依赖「前 3 次增强自动展开」的推测性引导，实际反馈是模式切换让界面状态不可预期。现已收敛为单一完整配置界面，并删除旧偏好读取，避免同一入口出现不同布局。

### 升级注意

- 自定义宿主若此前注入了 `searchFiles`，可直接移除注入；保留不报错，但不再有任何效果。Embed Protocol v1 其余接口不变。
- 自定义宿主若依赖「界面模式」设置引导新用户，注意该设置与旧存储键均已移除；当前只有单一完整界面。
