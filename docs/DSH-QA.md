...
6. 输入 `@关键词`，检查结果、Enter 插入和 IME 组合输入；不得重复 `@` 或误发送。大仓库受限时显示索引不完整提示。
7. 桌面和 390×844 窄屏检查面板、抽屉、关闭按钮是否在视口内；检查相关控制台警告。
8. 恢复原草稿与原显示偏好，仅清除本次测试产生的暂存发现；保留原有资产。

补充边界：切换轻量/语义后在面板内用 ⌘/Ctrl+Enter；⌘/Ctrl+数字须使用刚选的方法。切换工作区后文件候选必须来自当前会话目录。只有普通 textarea 嵌入入口支持的选区/自动发送能力，用其集成测试验证，不能把 DSH 未注入的能力当成真机覆盖。
