...
test('灵感库关闭：抽屉内「引用已选对话」后点 ×，主面板必须保留（回归：quoteSelectedMessages 曾误 setOpen(false)）', async () => {
  ...
  contextBtn.dispatchEvent(new W.MouseEvent('click', { bubbles: true }))
  await tick()
  const pickBtn = [...container.querySelectorAll('button')].find(b => /全选/.test(b.textContent))
  assert.ok(pickBtn, '对话选择层应出现批量选择按钮')
  pickBtn.dispatchEvent(new W.MouseEvent('click', { bubbles: true }))
  await tick()
  const confirmBtn = [...container.querySelectorAll('button')].find(b => /确认选择/.test(b.textContent))
  assert.ok(confirmBtn, '对话选择层应出现确认按钮')
